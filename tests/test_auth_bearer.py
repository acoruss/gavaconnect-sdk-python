...
class MockTokenProvider:
    ...
    def __init__(self, token: str = "test_token", refresh_token: str = "new_token"):
        self.token = token
...
class TestBearerAuthPolicy:
    ...
    async def test_full_flow(self):
        ...
        provider = MockTokenProvider(token="initial_token", refresh_token="refreshed_token")
        policy = BearerAuthPolicy(provider)
...
class TestBearerAuthPolicyIntegration:
    ...
    async def test_integration_refresh_flow(self):
        ...
        assert request2.headers["authorization"] == "Bearer token_v2"  # Uses cached refreshed token
